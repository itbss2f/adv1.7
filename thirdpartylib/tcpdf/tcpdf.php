...
class TCPDF {
	...
	public function getPageSizeFromFormat($format) {
		...
		switch (strtoupper($format)) {
			...
			case 'ORGANIZERM':
			case 'USLETTER':
			case 'LETTER' : {$pf = array(  595.000,  792.000); break;}
			case 'INVOICEPAPER' : {$pf = array(  612.000,  504.000); break;}
			case 'USLEGAL':
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			$pagenumtxt = 'Run Date: '.$this->l['w_page'].date('M d Y H:i:s').' '.$this->getAliasNumPage().' / '.$this->getAliasNbPages();
		} else {
			$pagenumtxt = 'Run Date: '.$this->l['w_page'].date('M d Y H:i:s').' '.$this->getPageNumGroupAlias().' / '.$this->getPageGroupAlias();
		}
		$this->SetY($cur_y);
		//Print page number
		if ($this->getRTL()) {
