...
class Dummy_Lib {
    ...
    function create_layout($color, $papersize, $constant, $data, $randname) {
        ...
        imageline ($im , 250 , 300 , $pw_innerborder , 300 , $darkgrey);                             # top
        imageline ($im , 250 , $pl_innerborder , $pw_innerborder , $pl_innerborder , $darkgrey);     # bottom
        imageline ($im , 250 , 300 , 250 , $pl_innerborder , $darkgrey);                             # left      
        imageline($im , $pw_innerborder , 300 , $pw_innerborder , $pl_innerborder , $darkgrey);      # right
        ...
        for ($c = 1; $c < $cons['column']; $c++) {
            ...
            imagedashedline($im, $col_new_point - $cons['gutter_per_pixel'], 300, $col_new_point - $cons['gutter_per_pixel'], $paper_size['length'] - 202, $grey);    
            imagedashedline($im, $col_new_point, 300, $col_new_point, $paper_size['length'] - 202, $grey);        
            $col_new_point += $cols;
        } 
        ...
        */   
        ...
        $cm_point = 230;
        $centi = $cons['cm_per_pixel'];
        $cm_new_point = $cm_point + $centi;
        $centiledger = $cons['centimeter'];

        $font = dirname(__FILE__) . '/fonts/arial.ttf';      
        $style = array($white, $white, $white, $grey, $grey, $grey);
        imagesetstyle($im, $style);
        
        imagettftext($im, 25, 0, 250, 280, $black, $font, $data['pageprint']['class_code'].'-'.$data['pageprint']['folio_number']);    
        imagettftext($im, 25, 0, ($paper_size['width'] / 2) - 70, 280, $black, $font, 'Page '.$data['pageprint']['book_name'].'-'.$data['pageprint']['folio_number']);    
        for ($cm = 1 ; $cm <= $cons['centimeter'] + 1; $cm++) {
        ...
        imagettftext($im, 25, 0, ($paper_size['width'] / 2) - 70, 4050, $black, $font, $data['pageprint']['color_code']); 
        ...
        for ($b = 0; $b < $countbox; $b++) {
            $box_x1 = (($data['boxprint'][$b]['xaxis'] / 35) * $cons['col_per_pixel']) + ((($data['boxprint'][$b]['xaxis'] / 35)) * $cons['gutter_per_pixel']); 
            $box_y1 = ($data['boxprint'][$b]['yaxis'] / 10) * $cons['cm_per_pixel'];       
        ...
        imagettftext($im, 25, 0, $paper_size['width'] - 350, 280, $black, $font, round($pagepercent, 2).'%');     

        #imagejpeg($im, "/tmp/dummy_layout_output/".$randname."".$data['pageprint']['folio_number'].".jpg");
        imagejpeg($im, "D:\\test\\".$randname."".$data['pageprint']['folio_number'].".jpg");
        imagedestroy($im);         
