...
<div class="pagecon" id="pageconpagex<?php echo $pages[$a]['layout_id'] ?>" align="center" style="width: <?php echo $pages[$a]['columnpixel'] ?>px; height: <?php echo $pages[$a]['lenpixel'] + 20 ?>px;">
	<div class="pagediv" id="pagex<?php echo $pages[$a]['layout_id'] ?>" style="position: relative; width: <?php echo $pages[$a]['columnpixel'] ?>px; height: <?php echo $pages[$a]['lenpixel']?>px; <?php echo $color ?>; background: <?php echo $pages[$a]['pagecolor'] ?> url(<?php echo base_url(); ?>assets/images/dummy/<?php echo $grid ?>);">
		<!-- Boxes inside -->
		<script>			
			$.ajax({
				url: '<?php echo site_url('classdummy/dummy/ajxRetMyOwnBox') ?>',
				type: 'post',
				...
				success: function(response) {
					...
					$.each($response['box'], function(i){
						...
						if ($item['component_type'] == "blockout") {
                            ...
                            var $title = $item['ao_num']+' '+ $item['ao_payee'] +' '+ $item['colx'] +' x '+ $item['lenx']+' '+ $item['ao_part_records']+' '+ $item['agencyname']+' '+ $item['agencycontacts']+' '+$item['inputdate'];  															
						                            
                            if ($item['is_lock'] == '1') {
                                var $box = "<div class='draggableboxlock' data-value='pr' title='"+$title+"' id='" + $item['layout_boxes_id'] + "' style='overflow:hidden;-width:100%;display:block;outline:3px dotted #000000;outline-offset:-2px;position: absolute; z-index: 20; width:" + $item['columnpixel'] + "px;  height: " + $item['lenpixel'] + "px; left: " + $xpos + "px; top: " + $ypos + "px;" + $divcolor +";<?php echo $font ?>'><p>"+$item['ao_num']+" "+ $item['ao_payee'] + " " + $item['colx'] + " x " + $item['lenx'] + "</p></div>";                                        
                            }  else {
                                var $box = "<div class='draggablebox' data-value='pr' title='"+$title+"' id='" + $item['layout_boxes_id'] + "' style='overflow:hidden;-width:100%;display:block;position: absolute; z-index: 20; width:" + $item['columnpixel'] + "px;  height: " + $item['lenpixel'] + "px; left: " + $xpos + "px; top: " + $ypos + "px;" + $divcolor +";<?php echo $font ?>'><p>"+$item['ao_num']+" "+ $item['ao_payee'] + " " + $item['colx'] + " x " + $item['lenx'] + "</p></div>";
                            }  
                        }
						...
						$("#"+ $item['layout_boxes_id'] +"").click(function(){		
							...
							if (selection == "Unflow") {
								$.ajax({
									url: "<?php echo site_url('classdummy/dummy/unflow') ?>",
									type: "post",
...
<script type="text/javascript">	
	...
	{
				$.ajax({
					url: '<?php echo site_url('classdummy/dummy/ajxPgBxVal') ?>',
					type: 'post',
	...
	$(".pagediv").click(function() {
		...
		if (selection == "" || selection == "Unflow" || selection == "Remarks" || selection == "Lockbox" || selection == "Unlockbox") {			
			...
			$.ajax({
				url: '<?php echo site_url('classdummy/dummy/ajxtSetValue') ?>',
				type: 'post',
	...
	$(".pagediv").dblclick(function() {
		var pageID = $(this).attr("id");
		$.ajax({
			url: '<?php echo site_url('classdummy/dummy/openFolio') ?>',
			type: 'post',
    ...
    function productionremarks(id) {
        var boxid = id;        
        $.ajax({
            url: "<?php echo site_url('classdummy/dummy/productremarksview') ?>",
            type: 'post',
            data: {boxid: boxid},
            success: function(response) {
                var $response = $.parseJSON(response);
    ...
    function do_lockbox(id) {    
        //Do is_lock to 2           
        $.ajax({
            url: "<?php echo site_url('classdummy/dummy/do_lock') ?>",
            type: 'post',
            data: {boxid: id},
            success:function(response) {
    ...
    function do_unlockbox(id) {    
        //Do is_lock to 3        
        $.ajax({
            url: "<?php echo site_url('classdummy/dummy/do_unlock') ?>",
            type: 'post',
            data: {boxid: id},
            success:function(response) {
                $('#'+id).draggable();
