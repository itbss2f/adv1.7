...
class Dummies extends CI_Model {
    ...
    public function class_header() {
        $stmt = "SELECT id, class_name FROM misclass ORDER BY class_name ASC";
        ...
        if ($chckresult['total'] < 0) {
            ...
            foreach ($result as $row) {                      
                $template[] = array('layout_sequence' => $row['layout_sequence'],
                                    ...
                                    'amount_ads' => $row['amount_ads'], 'status' => $row['status'], 'status_d' => date('Y-m-d h:m:s'),
                                    'user_n' => $userid , 'edited_n' => $userid, 'edited_d' => date('Y-m-d h:m:s'));
            }
    ...
    public function getBoxPrint($date, $layout_id, $product) {
        $stmt = "SELECT d_layout_boxes.box_width, d_layout_boxes.box_height, d_layout_boxes.xaxis, d_layout_boxes.yaxis, ao_p_tm.ao_color,
                        CONCAT(d_layout_boxes.box_width, ' x ', d_layout_boxes.box_height) AS boxsize,
                        ao_m_tm.ao_payee, CONCAT('AO No. ', ao_m_tm.ao_num) AS ao_num, IFNULL(miscolor.color_code, '') AS color_name,
						   d_layout_boxes.box_description, d_layout_boxes.component_type
                 FROM d_layout_boxes
    ...
    public function getPagePrint($date, $product) {
        $stmt = "SELECT d_layout_pages.layout_id, d_layout_pages.book_name, IFNULL(d_layout_pages.class_code, 'No Class') AS class_code, d_layout_pages.folio_number ,
                        miscolor.color_name AS color_code
                 FROM d_layout_pages 
	...
	{
		$stmt = "SELECT layout_id, IFNULL(color_code, 0) as color_code
				 FROM temp_layout_page_c WHERE hkey = '".$data['key']."' AND layout_id = '".$data['mergedID']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."'";		
	...
	public function mergePage($data) {		
		...
		$stmt = "UPDATE temp_layout_page_c SET is_merge = '".$data['merge']."' , is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$data['page']."'";		
		$this->db->query($stmt);
		$nextpage = $data['page'] + 1;
		$stmt2 = "UPDATE temp_layout_page_c SET is_merge = 'x' , is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$nextpage."'";		
		$this->db->query($stmt2);
		return true;
	}

	public function unmergePage($data) {
		$stmt = "SELECT is_merge
				 FROM temp_layout_page_c WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."'";		
		$result = $this->db->query($stmt)->row_array();
		if(!empty($result)) {
			$stmt2 = "UPDATE temp_layout_page_c SET is_merge = NULL , is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$result['is_merge']."'";		
			$this->db->query($stmt2);
		}
		
		$stmt3 = "UPDATE temp_layout_page_c SET is_merge = NULL , is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$data['page']."'";		
		$this->db->query($stmt3);
		return true;
	}
	...
	public function getPageData($data) {
		$stmt = "SELECT layout_id, layout_sequence, book_name, class_type, class_code, prod_code, 
				 ...
				 FROM temp_layout_page_c WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' 
	...
	public function insertBlockout($data){
		...
		$this->db->insert('temp_layout_box_c', $datablockout);
		return true;						
	}
	...
	public function setFolioNumber($data){
		...
		if ($data['checkfolio'] == 1) {
			...
			$condition = "AND folio_number >= (SELECT folio_number FROM temp_layout_page_c WHERE  hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$data['page']."')";
		} else {
			$condition = "AND layout_id = '".$data['page']."'";
		}
		//print_r2($data);
		$stmt = "SELECT layout_id, folio_number FROM temp_layout_page_c WHERE  hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' $condition 
                AND book_name = (SELECT book_name FROM temp_layout_page_c WHERE  hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$data['page']."')
                ORDER BY book_name, folio_number ASC;";
		$result = $this->db->query($stmt)->result_array();
		$countres = COUNT($result);
		$fn = $data['folionumber'];			
			for ($x = 0; $x < $countres; $x++) {				
				$stmtupd = "UPDATE temp_layout_page_c SET folio_number = '".$fn."', is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$result[$x]['layout_id']."' ";				
	...
	public function unflowBoxAds($data) {
		$stmtdeltemp = "DELETE FROM temp_layout_box_c WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_boxes_id = '".$data['boxid']."'";
	...
	public function deletePage($data) {
		$stmt = "DELETE FROM temp_layout_page_c WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."'";
		$this->db->query($stmt);
	...
	public function checkPageHasAds($data) {		
		$stmt = "SELECT COUNT(hkey) AS result FROM temp_layout_box_c WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$data['page']."'";
		$result = $this->db->query($stmt)->row();
		return $result->result;
	}

	public function validatePageToChangeColor($data, $color_rank) {
		$stmt = "SELECT IFNULL(c.color_rank, 0) AS color_rank, b.id, b.ao_color FROM temp_layout_box_c AS a
	...
	public function setPageSection($data){
		$stmt = "UPDATE temp_layout_page_c SET class_code = '".$data['section']."', is_change = '1' WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."'";
		$this->db->query($stmt);
		...
		if(!empty($result)) {
			$stmt3 = "UPDATE temp_layout_page_c SET class_code = '".$data['section']."', is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$result['is_merge']."'";		
			$this->db->query($stmt3);
		}
		return true;
	}
	...
	public function setPageColor($data){
		$stmt = "UPDATE temp_layout_page_c SET color_code = '".$data['color_code']."', color_html = '".$data['color_html']."', is_change = '1' WHERE hkey = '".$data['key']."' AND layout_id = '".$data['page']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."'";		
		$this->db->query($stmt);
		...
		$result = $this->db->query($stmt2)->row_array();
		if(!empty($result)) {
			$stmt3 = "UPDATE temp_layout_page_c SET color_code = '".$data['color_code']."', color_html = '".$data['color_html']."', is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$result['is_merge']."'";		
			$this->db->query($stmt3);
		}
		return true;
	}
	...
	public function addNewPage($data) {
		...
		$otherinfo = $this->db->query("SELECT `columns`, len, (`columns` * 30) + ((`columns` - 1) * 5) AS columnpixel, (len * 10) AS lenpixel 
		...
		//$layfolio_number = $this->db->query("SELECT IFNULL(MAX(folio_number), 0) AS folio_number FROM temp_layout_page_c ")->row_array();
		$layid =  $this->db->query("SELECT IFNULL(MAX(layout_id), 0) AS layout_id FROM temp_layout_page_c")->row_array();
		$lid = $layid['layout_id'];
		for ($x = 0; $x < $data['numberofpage']; $x++) {			
			$lid += 1;
			$dataque['layout_id'] = $lid;
			
			$this->db->insert('temp_layout_page_c', $dataque);			
		}
		
		$stmtupfolio = $this->db->query("SELECT layout_id FROM temp_layout_page_c WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' 
		...
		for ($z = 0; $z < $countfol; $z++) {			
			$stmtupd = "UPDATE temp_layout_page_c SET folio_number = '".$seqfol."', layout_sequence = '".$seqfol."', is_change = '1' WHERE hkey = '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$stmtupfolio[$z]['layout_id']."' ";				
			$this->db->query($stmtupd);
			$seqfol += 1;          
		}
	...
	{
		$stmt = "SELECT box_id FROM temp_layout_box_c 
					WHERE 
	...
	public function getBoxLvl($data) {
		...
		if (!empty($result)) {
			...
			$stmtfind = "SELECT component_type FROM temp_layout_box_c WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_boxes_id = '".$data['box']."' AND component_type ='blockout'";
			...
			foreach ($result as $row) {
				...
				if (empty($exist)) {
					$datainsert = array('temp_layout_id' => $row['layout_id'],'layout_sequence' => $row['layout_sequence'], 'prod_code' => $row['prod_code'], 'book_name' => $row['book_name'], 
										...
										'edited_n' => $userid, 'edited_d' => date('Y-m-d h:m:s'));
					...
					$stmtupd = "UPDATE temp_layout_page_c SET is_change = '0' WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_id = '".$row['layout_id']."'";					
                    $this->db->query($stmtupd);                    
                    $this->db->query($stmtupdbox);                    
                    $this->db->query($stmtupdpage);                    
				} else {
					$dataupdate = array('layout_sequence' => $row['layout_sequence'], 'folio_number' => $row['folio_number'], 'page_number' => $page_number, 'class_code' => $row['class_code'], 'color_code' => $row['color_code'], 'edited_n' => $userid, 'edited_d' => date('Y-m-d h:m:s'), 'is_merge' => $row['is_merge']);
			...
			foreach ($result as $row) {
				...
				if (empty($exist)) {
					$datainsert = array('temp_layout_id' => $row['layout_id'], 'prod_code' => $row['prod_code'], 'issuedate' => $row['issuedate'], 
										...
										'edited_n' => $userid, 'edited_d' => date('Y-m-d h:m:s'), 'xaxis' => $row['xaxis'], 'yaxis' => $row['yaxis'],
										'width' => $row['width'], 'height' => $row['height'], 'box_width' => $row['columns'], 'box_height' => $row['len']);
					$this->db->insert('d_layout_boxes', $datainsert);					
					$stmtupd = "UPDATE temp_layout_box_c SET is_change = '0' WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_boxes_id = '".$row['layout_boxes_id']."'";
					$this->db->query($stmtupd);
				} else {					
					...
					$stmtupd2 = "UPDATE temp_layout_box_c SET is_change = '0' WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_boxes_id = '".$row['layout_boxes_id']."'";
	...
	{
		$stmt = "SELECT COUNT(is_change) AS changes FROM temp_layout_box_c WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND is_change = '1'";
		$result = $this->db->query($stmt)->row_array();
		return $result;
	}
	...
	{
		$stmt = "SELECT COUNT(is_change) AS changes FROM temp_layout_page_c WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND is_change = '1'";
		$result = $this->db->query($stmt)->row_array();
		return $result;
	}
	...
	public function validateBox($data)
	{
		$stmt = "SELECT layout_boxes_id, layout_id
				 FROM temp_layout_box_c WHERE hkey= '".$data['key']."' AND prod_code = '".$data['product']."' AND DATE(issuedate) = '".$data['date']."' AND layout_boxes_id = '".$data['box']."'";
	...
	public function getBox($data)
	{
		...
		$stmt = "SELECT a.layout_boxes_id, a.ao_num, a.component_type, a.box_description, a.issuedate, a.prod_code, a.xaxis, a.yaxis, a.width, 
				       a.height, ((a.columns * $col) + ((a.columns - 1) * $gutter)) AS columnpixel, (a.len * $cm) AS lenpixel, IFNULL(c.color_html, '918F8F') AS color_html,
	...
	public function getMyBox($data)
	{
		...
		$stmt = "SELECT a.layout_boxes_id, a.ao_num, a.component_type, a.box_description, a.issuedate, a.prod_code, a.xaxis, a.yaxis, a.width, 
				       a.height, ((a.columns * $col) + ((a.columns - 1) * $gutter)) AS columnpixel, (a.len * $cm) AS lenpixel, IFNULL(c.color_html, '918F8F') AS color_html,
				       aom.ao_payee, aop.ao_width AS colx, aop.ao_length as lenx, a.columns, a.len, aop.is_lock,
                       aop.ao_part_records,
	...
	public function updateBoxPosition($data)
	{
		...
		$this->db->update('temp_layout_box_c', $data2);
		return "U";
	}
	...
	{			
		...
		if (!empty($validate)) {
			...
			$this->db->insert('temp_layout_box_c', $data2);
			return "I";
		}
	}
	...
	public function findThisAOPTMData($data)
	{
		$stmt = "SELECT p.prod_name, a.ao_num, ao_part_billing,
						((a.ao_width * 30) + ((a.ao_width - 1) * 5)) AS width,
					    (a.ao_length * 10) AS height, a.ao_width as w, a.ao_length as h			
				FROM ao_p_tm AS a      
	...
	public function getTempPage($hkey, $product, $date, $viewing)
	{
		...
		$stmt = "SELECT a.hkey, a.layout_id, a.layout_sequence, a.book_name, a.class_type, a.class_code, a.color_code, a.folio_number,
                        ...
                        IFNULL(cl.class_htmlcolor, '#FFFFFF') AS pagecolor
				 FROM temp_layout_page_c AS a     
				 LEFT OUTER JOIN temp_layout_page_c AS b 
	...
	{
		$stmt = "INSERT INTO temp_layout_box_c (SELECT CONCAT('".$hkey."') AS hkey, a.id, a.layout_id, a.prod_code, p.prod_name, a.issuedate, a.component_type, a.ao_num,
		...
		$stmtreturnisflowto0whenlayoutbutnotsave = "UPDATE ao_p_tm SET is_flow = '0' WHERE is_flow = '1' AND ao_prod = '".$product."' AND DATE(ao_issuefrom) = '".$date."' AND id NOT IN(SELECT layout_boxes_id FROM d_layout_boxes WHERE prod_code = '$product' AND DATE(issuedate) = '$date')";
		...
		$stmtreturnisflowto2 = "UPDATE ao_p_tm SET is_flow = '2' WHERE id IN(SELECT layout_boxes_id FROM d_layout_boxes WHERE prod_code = '".$product."' AND DATE(issuedate) = '".$date."')";
	...
	{		
		$stmt = "INSERT INTO temp_layout_page_c (SELECT CONCAT('".$hkey."') AS hkey, a.layout_id, a.layout_sequence, b.book_name, a.class_type, a.class_code, 
				a.prod_code, a.issuedate, c.id as color, a.folio_number, c.color_html, b.columns, b.len, 
				((b.columns * 30) + ((b.columns - 1) * 5)) AS columnpixel,
				(b.len * 10) AS lenpixel, CONCAT('0') AS is_change, a.is_merge
	...
	public function retrieveAds($product, $date)
	{
		$stmt = "SELECT a.id, a.ao_num, DATE(a.ao_date) AS ao_date, a.ao_width, a.ao_length, a.ao_totalsize, a.ao_color, a.ao_position, 
				 ...
				 AND a.ao_type = 'C' ORDER BY a.user_d ASC";
	...
	{
		$stmt = "SELECT id, prod_code FROM misprod WHERE is_deleted='0' AND (prod_adtype = 'C' OR prod_adtype = 'B') ORDER BY prod_code ASC";
        $result = $this->db->query($stmt)->result_array();
        return $result;
	}
	...
	public function filtering($data)
	{
		...
		if ($data['show'] == "") 
		{
			$show = "AND (a.is_flow = '0' OR a.is_flow = '2')" ;
			
		} else if ($data['show'] == "0") {
			$show = "AND a.is_flow = '".$data['show']."'";
		} else if ($data['show'] == "2") {$show = "AND (a.is_flow = '1' OR a.is_flow = '2')" ;     }		
		
		$stmt = "SELECT a.id, a.ao_num, DATE(a.ao_date) AS ao_date, a.ao_width, a.ao_length, a.ao_totalsize, a.ao_color, a.ao_position, a.ao_part_records, 
					   a.ao_pagemin, a.ao_pagemax, a.ao_grossamt, b.color_code, b.color_html, a.status, m.ao_cmf, m.ao_payee, c.class_code, a.is_flow
				 FROM ao_p_tm AS a
				 INNER JOIN ao_m_tm AS m ON a.ao_num = m.ao_num
				 LEFT OUTER JOIN misclass AS c ON c.id = a.ao_class
				 LEFT OUTER JOIN miscolor AS b ON a.ao_color = b.id
			 WHERE a.ao_prod = '".$data['product']."' AND DATE(a.ao_issuefrom) = '".$data['date']."' $show
				 	AND a.ao_type = 'D' $section $color $name $code $aonum $width $height";
		$result = $this->db->query($stmt)->result_array();
		return $result;
	}
