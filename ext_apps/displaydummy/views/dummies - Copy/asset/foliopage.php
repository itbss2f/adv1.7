...
<div id="newtabs" style="background: #A4A4A4">
	...
	<div id="newtab1">
		...
		<p> <input type="checkbox" name="checkfolio" id="checkfolio" value="1">
...
<script>
	...
	$("#okfolio").click(function(){
		if ($("#folionumber").val() != "") {
			$.ajax({
				url: '<?php echo site_url('dummy/ajxSetFolio') ?>',
				type: 'post',
	...
	$("#okblockout").click(function(){		
			...
			if (reservewidth == "" || reserveheight == "" || reservedescription == "") {
				...
				$.ajax({
					url: '<?php echo site_url('dummy/addblockout') ?>',
					type: 'post',
					data: {key: '<?php echo $key ?>', product: '<?php echo $product ?>', date: '<?php echo $date ?>', 
	...
	$("#mergepage").click(function(){
		$.ajax({
			url: '<?php echo site_url('dummy/ajxmergePage') ?>',
			type: 'post',
			data: {key: '<?php echo $key ?>', product: '<?php echo $product ?>', date: '<?php echo $date ?>', 
				   pageID: '<?php echo $pageID ?>', viewing: '<?php echo $viewing ?>',
				   mergedID: <?php echo $pageData['folio_number'] + 1 ?>},
			success: function(response) {	
				var $response = $.parseJSON(response);
	...
	$("#unmergepage").click(function(){
		$.ajax({
			url: '<?php echo site_url('dummy/ajxUnmergePage') ?>',
			type: 'post',
			data: {key: '<?php echo $key ?>', product: '<?php echo $product ?>', date: '<?php echo $date ?>', 
				   pageID: '<?php echo $pageID ?>', viewing: '<?php echo $viewing ?>'},
			success: function(response) {	
