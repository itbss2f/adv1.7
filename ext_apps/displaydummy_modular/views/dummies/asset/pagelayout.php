...
<div class="pagecon" id="pageconpagex<?php echo $pages[$a]['layout_id'] ?>" align="center" style="width: <?php echo $pages[$a]['columnpixel'] ?>px; height: <?php echo $pages[$a]['lenpixel'] + 20 ?>px;">
	<div class="pagediv" data-page="<?php echo $pages[$a]['in_page'] ?>" id="pagex<?php echo $pages[$a]['layout_id'] ?>" style="position: relative; width: <?php echo $pages[$a]['columnpixel'] ?>px; height: <?php echo $pages[$a]['lenpixel']?>px; <?php echo $color ?>; background: <?php echo $pages[$a]['pagecolor'] ?> url(<?php echo base_url(); ?>assets/images/dummy/<?php echo $grid ?>);">
		...
		<script>			
			$.ajax({
				url: '<?php echo site_url('displaydummy_modular/dummy/ajxRetMyOwnBox') ?>',
				type: 'post',
						...
						$("#"+ $item['layout_boxes_id'] +"").click(function(){	
                                ...
                                if (selection == "Unflow") {
                                    ...
                                    if (unfl == "draggableboxpage") {
                                        ...
                                        $.ajax({
                                            url: "<?php echo site_url('displaydummy_modular/dummy/unflow') ?>",
                                            type: "post",
                               ...
                               if (selection == "Material") {
                                   window.open("<?php echo site_url('displaydummy_modular/dummy/material') ?>/"+boxid, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=600, height=500");
                               }
...
<script type="text/javascript">	
	...
	{
				$.ajax({
					url: '<?php echo site_url('displaydummy_modular/dummy/ajxPgBxVal') ?>',
					type: 'post',
	...
	$(".pagediv").click(function() {
		...
		if (selection == "" || selection == "Unflow" || selection == "Remarks" || selection == "Lockbox" || selection == "Unlockbox") {			
			...
			$.ajax({
				url: '<?php echo site_url('displaydummy_modular/dummy/ajxtSetValue') ?>',
				type: 'post',
	...
	$(".pagediv").dblclick(function() {
		...
		$.ajax({
			url: '<?php echo site_url('displaydummy_modular/dummy/openFolio') ?>',
			type: 'post',
    ...
    function productionremarks(id) {
        var boxid = id;        
        $.ajax({
            url: "<?php echo site_url('displaydummy_modular/dummy/productremarksview') ?>",
            type: 'post',
            data: {boxid: boxid},
            success: function(response) {
                var $response = $.parseJSON(response);
    ...
    function do_lockbox(id) {    
        //Do is_lock to 2           
        $.ajax({
            url: "<?php echo site_url('displaydummy_modular/dummy/do_lock') ?>",
            type: 'post',
            data: {boxid: id},
            success:function(response) {
    ...
    function do_unlockbox(id) {    
        //Do is_lock to 3        
        $.ajax({
            url: "<?php echo site_url('displaydummy_modular/dummy/do_unlock') ?>",
            type: 'post',
            data: {boxid: id},
            success:function(response) {
                $('#'+id).draggable();
