...
{	
	...
	$.ajax({
		url: '<?php echo site_url('displaydummy_modular/dummy/ajxLayBxTPg') ?>',
		type: 'post',
...
function updateBoxPosition(product, date, key, page, box, xpos, ypos) {
	$.ajax({
		url: '<?php echo site_url('displaydummy_modular/dummy/ajxUpdatePost') ?>',
		type: 'post',
		data: {product: product, date: date, key: key, page: page, box: box, xpos: xpos, ypos: ypos, viewing: $("#viewing").val()},
		success: function(response) {
            var $response = $.parseJSON(response);   
...
function allocatedAreaValidate(event, ui, box, xpos, ypos, width, height) {
	$.ajax({
		url: '<?php echo site_url('displaydummy_modular/dummy/ajxAllocAreaVal') ?>',
		type: 'post',
