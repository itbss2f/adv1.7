...
class Dummies extends CI_Model {        
    ...
    public function getBoxPrintz($date, $layout_id, $product, $cols) {
        $stmt = "SELECT IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) AS box_width,
                    ...
                    (d_layout_boxes.xaxis + (IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) * 30) + ((IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) - 1) * 5)) AS totalbw
                FROM d_layout_boxes
                ...
                AND (d_layout_boxes.xaxis + (IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) * 30) + ((IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) - 1) * 5)) <= (($cols * 30) + (($cols - 1) * 5))
                UNION
                SELECT (((($cols * 30) + (($cols - 1) * 5))- d_layout_boxes.xaxis) + 5) / 35 AS box_width,
                    d_layout_boxes.box_height, d_layout_boxes.xaxis, d_layout_boxes.yaxis, ao_p_tm.ao_color,
                    CONCAT(d_layout_boxes.box_width, ' x ', d_layout_boxes.box_height) AS boxsize,
                    IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.box_description ,ao_m_tm.ao_payee) AS ao_payee, 
                    IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.ao_num ,CONCAT('AO No. ', ao_m_tm.ao_num)) AS ao_num, 
                ...
                WHERE DATE(d_layout_boxes.issuedate) = '$date' AND layout_id = '$layout_id' AND prod_code = '$product'
                AND (d_layout_boxes.xaxis + (IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) * 30) + ((IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) - 1) * 5)) >= (($cols * 30) + (($cols - 1) * 5))
                AND d_layout_boxes.xaxis < (($cols * 30) + (($cols - 1) * 5))
                AND (d_layout_boxes.xaxis + ((d_layout_boxes.box_width * 30) + ((d_layout_boxes.box_width - 1) * 5)) - (($cols * 30) + (($cols - 1) * 5))) / 35 <> 0"; 
    ...
    public function getBoxPrintx($date, $layout_id, $product, $cols) {    
            $stmt = "SELECT IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) AS box_width,
                        d_layout_boxes.box_height, (d_layout_boxes.xaxis - (($cols * 30) + (($cols) * 5))) AS xaxis, d_layout_boxes.yaxis, ao_p_tm.ao_color,
                        CONCAT(d_layout_boxes.box_width, ' x ', d_layout_boxes.box_height) AS boxsize,
                        IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.box_description ,ao_m_tm.ao_payee) AS ao_payee, 
                        IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.ao_num ,CONCAT('AO No. ', ao_m_tm.ao_num)) AS ao_num, 
                        IFNULL(miscolor.color_code, '') AS color_name,
                    ...
                    AND (xaxis + ((d_layout_boxes.box_width * 30) + ((d_layout_boxes.box_width - 1) * 5))) >= 310
                    AND d_layout_boxes.xaxis >= (($cols * 30) + (($cols - 1) * 5)) 

                    UNION 

                    SELECT z.box_width, z.box_height, IF (z.bbx > $cols, 0 , (z.totalbw - (z.box_width * (30 + 5)) - (($cols * 30) + (($cols - 1) * 5)))) AS xaxis, z.yaxis, z.ao_color, z.boxsize, z.ao_payee, z.ao_num, z.color_name, z.box_description, z.component_type, z.ao_eps
                    FROM (
                    SELECT (d_layout_boxes.xaxis + ((d_layout_boxes.box_width * 30) + ((d_layout_boxes.box_width - 1) * 5)) - (($cols * 30) + (($cols - 1) * 5))) / 35 AS box_width,
                        d_layout_boxes.box_height, d_layout_boxes.xaxis, d_layout_boxes.yaxis, ao_p_tm.ao_color,
                        CONCAT(d_layout_boxes.box_width, ' x ', d_layout_boxes.box_height) AS boxsize,
                        IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.box_description ,ao_m_tm.ao_payee) AS ao_payee, 
                        IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.ao_num ,CONCAT('AO No. ', ao_m_tm.ao_num)) AS ao_num, 
                    ...
                    AND (d_layout_boxes.xaxis + (IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) * 30) + ((IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) - 1) * 5)) >= (($cols * 30) + (($cols - 1) * 5))
                    AND d_layout_boxes.xaxis < (($cols * 30) + (($cols - 1) * 5))
                    AND (d_layout_boxes.xaxis + ((d_layout_boxes.box_width * 30) + ((d_layout_boxes.box_width - 1) * 5)) - (($cols * 30) + (($cols - 1) * 5))) / 35 <> 0) AS z";
    ...
    public function getBoxPrint($date, $layout_id, $product, $cols) {
            ...
            $stmt = "SELECT IF (d_layout_boxes.box_width > $cols, $cols, d_layout_boxes.box_width) AS box_width, d_layout_boxes.box_height, d_layout_boxes.xaxis, d_layout_boxes.yaxis, ao_p_tm.ao_color,
                            CONCAT(d_layout_boxes.box_width, ' x ', d_layout_boxes.box_height) AS boxsize,
                            IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.box_description ,ao_m_tm.ao_payee) AS ao_payee, 
                            IF (d_layout_boxes.component_type = 'blockout', d_layout_boxes.ao_num ,CONCAT('AO No. ', ao_m_tm.ao_num)) AS ao_num, 
                            IFNULL(miscolor.color_code, '') AS color_name,
	...
	public function addNewPage($data) {
		...
		$otherinfo = $this->db->query("SELECT `columns_old` AS `columns`, len, (`columns_old` * 30) + ((`columns_old` - 1) * 5) AS columnpixel, (len * 10) AS lenpixel 
	...
	public function getBox($data)
	{
		...
		$stmt = "SELECT a.layout_boxes_id, a.ao_num, a.component_type, a.box_description, a.issuedate, a.prod_code, a.xaxis, a.yaxis, a.width, 
				       ...
				       aom.ao_payee, aop.ao_width AS colx, aop.ao_length as lenx, IF(aop.ao_amt = 0, 'x', 'y') AS amt
				FROM temp_layout_box AS a
	...
	{		
		$stmt = "INSERT INTO temp_layout_page (SELECT CONCAT('".$hkey."') AS hkey, a.layout_id, a.layout_sequence, b.book_name, a.class_type, a.class_code, 
				a.prod_code, a.issuedate, c.id as color, a.folio_number, c.color_html, b.columns_old AS `columns`, b.len, 
				((b.columns_old * 30) + ((b.columns_old - 1) * 5)) AS columnpixel,
				(b.len * 10) AS lenpixel, CONCAT('0') AS is_change, a.is_merge, '1'
	...
	public function listOfPages()
	{
		$stmt = "SELECT book_name, minpagecount, maxpagecount, page_depth, page_width,
					   ...
					   columns_old AS `columns`, column_width, gutter_width
				FROM d_book_master";		
