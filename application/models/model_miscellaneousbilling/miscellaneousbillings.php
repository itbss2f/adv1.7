<?php
    ...
    class Miscellaneousbillings Extends CI_Model
    {
         ...
         function saveinquiry($data)
        {
              ...
              foreach($data['ae_val'] as $key => $value)
                {
                    if(!empty($value))
                    {
                        ...
                        $kuery = "UPDATE ao_p_tm AS a
                                  INNER JOIN ao_m_tm AS b ON a.ao_num = b.ao_num
                                  set b.ao_aef ='".$new_val[0]."' where a.id = '".$new_val[1]."' ";
                        $this->db->query($kuery);
            ...
            if(count($data['class_val'])>0)
            {
                foreach($data['class_val'] as $key => $value)
                {
                    ...
                    {
                        $new_val = explode(" : ",$value);
                        $kuery = "UPDATE ao_p_tm set ao_class ='".$new_val[0]."' where id = '".$new_val[1]."' "; 
                        $this->db->query($kuery);
                    }
