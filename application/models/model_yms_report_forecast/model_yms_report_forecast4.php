...
class Model_yms_report_forecast4 extends CI_Model {
    ...
    private function report_query($val) {
        ...
        switch ($val['reporttype']) {
            ...
            case 1:
                $stmt = "
                SELECT DATE_FORMAT(a.ao_issuefrom, '%m/%d/%Y') AS issuedate, 
                       TRIM(a.ao_billing_prodtitle) AS prodtitle,     
                       CONCAT(a.ao_width, ' x ', a.ao_length) AS size,
                       m.ao_payee AS advertiser, cmf.cmf_name AS agency,  
                       color.color_code, IFNULL(color.id, 0) AS colorid, 
                       a.ao_num, a.ao_totalsize, adtype.adtype_code, aosub.aosubtype_code,
                       a.ao_part_records, a.ao_billing_remarks, edition.code, edition.name, param.vat_inclusive ,(prod.prod_ccm) AS totalpageccm                                
                FROM ao_p_tm AS a
                INNER JOIN ao_m_tm AS m ON m.ao_num = a.ao_num
                INNER JOIN misaosubtype AS aosub ON aosub.id = m.ao_subtype
                ...
                WHERE DATE(a.ao_issuefrom) >= '$date' AND DATE(a.ao_issuefrom) <= '$dateto'
                $con_product $con_exclude   
                AND (a.status != 'C' AND a.status != 'F') AND aosub.aosubtype_spontag = 1
                ORDER BY m.ao_payee, a.ao_issuefrom ASC";
                #echo "<pre>"; echo $stmt; exit;                 
                $result = $this->db->query($stmt)->result_array();
                
                $nwresult = array();
                foreach ($result as $row) {
                    $nwresult[$row['code'].' - '.$row['name']][$row['advertiser']][] = $row;
                }
                
                $stmt2 = "SELECT rate.edition, rate.printing_press, rate.circulation_copies, rate.newsprint_cost_rate,
                        ...
                        WHERE DATE(rate.period_covered_from) <= '$date' AND DATE(rate.period_covered_to) >= '$dateto' AND rate.edition = $product ORDER BY rate.printing_press DESC";
               #echo "<pre>"; echo $stmt2; exit;        
                $result2 = $this->db->query($stmt2)->result_array(); 
                
                $newresult = array('val' => $result2, 'page' => $nwresult);  
            ...
            case 3:
                ...
                SELECT DATE_FORMAT(a.ao_issuefrom, '%m/%d/%Y') AS issuedate, 
                       ...
                       m.ao_payee AS advertiser, cmf.cmf_name AS agency,  
                       color.color_code, IFNULL(color.id, 0) AS colorid, 
                       a.ao_num, IFNULL(a.ao_totalsize, 0) AS ao_totalsize, adtype.adtype_code, aosub.aosubtype_code,
                       a.ao_part_records, a.ao_billing_remarks, edition.code, edition.name, param.vat_inclusive ,(prod.prod_ccm) AS totalpageccm                                
                FROM ao_p_tm AS a
                INNER JOIN ao_m_tm AS m ON m.ao_num = a.ao_num
                LEFT OUTER JOIN misaosubtype AS aosub ON aosub.id = m.ao_subtype
                LEFT OUTER JOIN miscmf AS cmf ON cmf.id = m.ao_amf
                LEFT OUTER JOIN miscolor AS color ON color.id = a.ao_color
                LEFT OUTER JOIN misadtype AS adtype ON adtype.id = m.ao_adtype
                LEFT OUTER JOIN misprod AS prod ON prod.id = m.ao_prod                                      
                ...
                WHERE DATE(a.ao_issuefrom) >= '$date' AND DATE(a.ao_issuefrom) <= '$dateto'
                $con_product $con_exclude   
                AND (a.status != 'C' AND a.status != 'F') AND a.ao_amt != 0
                ORDER BY m.ao_payee, a.ao_issuefrom ASC";
                #echo "<pre>"; echo $stmt; exit;                 
                $result = $this->db->query($stmt)->result_array();
            ...
            /*case 3:
                $stmt = "
                    ...
                    LEFT OUTER JOIN misprod AS prod ON prod.id = m.ao_prod      
                    LEFT OUTER JOIN yms_edition AS edition ON edition.id = '$product'         
                    WHERE  DATE(a.ao_issuefrom) <= '$date' AND DATE(a.ao_issuefrom) >= '$dateto' AND m.ao_type = 'D' AND ((a.status != 'C' AND a.status != 'F') OR a.ao_sinum != 0)       
                    $con_product $con_exclude
                    GROUP BY a.id   
                    ORDER BY page.book_name ASC) AS xall
            ...
            case 6:
                ...
                $result = $this->db->query($stmt)->result_array(); 
                      ...
                      $stmt2 = "SELECT xall.name, xall.issuedate, xall.book_name, COUNT(xall.layout_id) AS pagetotal,
                                ...
                                GROUP BY page.book_name, page.layout_id
                                ORDER BY issuedate, book_name) AS xall
                                LEFT OUTER JOIN yms_product_budget_sales AS ymspds ON DATE(ymspds.issue_date) = DATE(xall.issuedate) 
                                GROUP BY xall.name, xall.issuedate, xall.book_name";                                    
        ...
        switch ($val['reporttype']) {
                ...
                foreach ($data["page"] as $z => $xx) {    
                    ...
                    if ($totalpercentage < 0) {
                     ...
                     $result[] = array(array("text" => "TOTAL", "bold" => true),
                                  ...
                                  array("text" =>  number_format($gtot_printincost, 2, ".", ","), "bold" => true, "style" => true),
                                  array("text" =>  $tstrcm, "bold" => true, "style" => true), 
                                  array("text" =>  number_format($ttcm_amount, 2, ".", ","), "bold" => true, "style" => true), 
                                  array("text" =>  $tstrpercentage, "bold" => true, "style" => true));
            ...
            case 11:
                ...
                $result[] = array(array("text" => "TOTAL", "bold" => true),
                                  ...
                                  array("text" =>  $ttotalcontmargin, "bold" => true, "style" => true), 
                                  array("text" =>  $ttotalpercentage, "bold" => true, "style" => true));';
            break;
