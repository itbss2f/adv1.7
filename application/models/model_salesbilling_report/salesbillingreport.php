...
class Salesbillingreport extends CI_Model {
    
    public function getBillingSales($datefrom, $dateto, $reporttype, $bookingtype, $toprank, $adtypefrom, $adtypeto, $productfrom, $productto, $branchfrom, $branchto, $toprank, $paytype, $classification) {
        
        #echo $classification; exit; 
        $conpaytype = ""; $conbookingtype = ""; $conclass = ""; 
        ...
        if ($reporttype == 1) {  
                ...
                LEFT OUTER JOIN (
                                ...
                                AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 AND m.ao_amf != 0  $conbookingtype $conpaytype $conclass 
                                GROUP BY m.ao_amf
                                ) AS xall ON xall.ao_amf = m.ao_amf
                WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto'
                AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 AND m.ao_amf != 0  $conbookingtype $conpaytype $conclass
                GROUP BY m.ao_amf, monissuedate 
                ORDER BY particulars ASC, monissuedate ASC";    
                ...
                $stmt = "SELECT SUM(p.ao_grossamt) AS totalamountsales, p.ao_num, m.ao_amf, MONTH(p.ao_issuefrom) AS monissuedate,
                                m.ao_cmf, m.ao_payee AS particulars, xall.totalamt, p.ao_class
                FROM ao_p_tm AS p 
                INNER JOIN ao_m_tm AS m ON m.ao_num = p.ao_num
                INNER JOIN misclass as a ON a.id = p.ao_class
                LEFT OUTER JOIN (
                ...
                LEFT OUTER JOIN (
                                ...
                                INNER JOIN ao_m_tm AS m ON m.ao_num = p.ao_num 
                                WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto' 
                                AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 $conbookingtype $conpaytype $conclass                                 
                                GROUP BY m.ao_cmf
                                ) AS xall ON xall.ao_cmf = m.ao_cmf
                WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto'
                AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 $conbookingtype $conpaytype $conclass                
                ...
                $stmt = "SELECT SUM(p.ao_grossamt) AS totalamountsales, p.ao_num, m.ao_amf, MONTH(p.ao_issuefrom) AS monissuedate,
                               m.ao_cmf, m.ao_payee AS particulars, xall.totalamt, ad.adtype_name, p.ao_class
                        FROM ao_p_tm AS p 
                        INNER JOIN ao_m_tm AS m ON m.ao_num = p.ao_num
                        ...
                        LEFT OUTER JOIN (
                                ...
                                INNER JOIN misadtype AS ad ON ad.id = m.ao_adtype
                                WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto' 
                                AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 AND ad.adtype_name >= '$adtypefrom' AND ad.adtype_name <= '$adtypeto' $conbookingtype $conpaytype $conclass   
                        ...
                        ORDER BY ad.adtype_name, particulars ASC,  monissuedate ASC";    
                ...
                $stmt = "SELECT SUM(p.ao_grossamt) AS totalamountsales, p.ao_num, m.ao_amf, MONTH(p.ao_issuefrom) AS monissuedate,
                               m.ao_cmf, m.ao_payee AS particulars, xall.totalamt, prod.prod_name, p.ao_class 
                        FROM ao_p_tm AS p 
                        INNER JOIN ao_m_tm AS m ON m.ao_num = p.ao_num
                        ...
                        LEFT OUTER JOIN (
                            ...
                            INNER JOIN misprod AS prod ON prod.id = m.ao_prod
                            WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto' 
                            AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 AND prod.prod_name >= '$productfrom' AND prod.prod_name <= '$productto' $conbookingtype $conpaytype $conclass  
                        ...
                        ORDER BY prod.prod_name, particulars ASC,  monissuedate ASC";
                #echo "<pre>"; echo $stmt; exit;        
                $result = $this->db->query($stmt)->result_array();
                ...
                $stmt = "SELECT SUM(p.ao_grossamt) AS totalamountsales, p.ao_num, m.ao_amf, MONTH(p.ao_issuefrom) AS monissuedate,
                               m.ao_cmf, m.ao_payee AS particulars, xall.totalamt, prod.prod_name, bran.branch_name, p.ao_class 
                        FROM ao_p_tm AS p 
                        INNER JOIN ao_m_tm AS m ON m.ao_num = p.ao_num
                        INNER JOIN misprod AS prod ON prod.id = m.ao_prod
                        ...
                        LEFT OUTER JOIN (
                            ...
                            INNER JOIN misbranch AS bran ON bran.id = m.ao_branch
                            WHERE DATE(p.ao_issuefrom) >= '$datefrom' AND DATE(p.ao_issuefrom) <= '$dateto' 
                            AND (p.status != 'C' AND p.status != 'F') AND p.ao_amt != 0 AND bran.branch_name >= '$branchfrom' AND bran.branch_name <= '$branchto' $conbookingtype $conpaytype $conclass   
                        ...
                        ORDER BY bran.branch_name, prod.prod_name, particulars ASC,  monissuedate ASC";
