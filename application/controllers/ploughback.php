...
class PloughBack extends CI_Controller {
    
    public function __construct() {
        parent::__construct();
        
        $this->load->model('model_ploughback/mod_ploughback');
    }
    ...
    public function index() {
        ...
        $welcome_layout['content'] = $this->load->view('ploughback/index', $data, true);
    ...
    public function generatereport($datefrom, $dateto, $reporttype, $vdays, $agencyfrom, $c_clientfrom, $ac_agency, $ac_client) {
        ...
        if ($reporttype == 1) {
            $reportname = "AGENCY";   
            $customerdata = $this->mod_ploughback->getCustomerData(1, $agencyfrom);
        } else if ($reporttype == 2) {
            $reportname = "CLIENT";           
            $customerdata = $this->mod_ploughback->getCustomerData(2, $c_clientfrom);  
        } else if ($reporttype == 3) {
            $reportname = "AGENCY CLIENT";   
            $customerdata = $this->mod_ploughback->getCustomerData(3, $ac_agency);            
        }
        
        $fields = array(
                            array('text' => 'Invoice Date', 'width' => .09, 'align' => 'center'),
                            array('text' => 'Invoice Number', 'width' => .09, 'align' => 'center'),
                            array('text' => 'Total Gross Amount', 'width' => .10, 'align' => 'center'),
                            array('text' => '15% A/C', 'width' => .08, 'align' => 'center'),
                            array('text' => 'Total Net Amount', 'width' => .12, 'align' => 'center'),
                            array('text' => 'OR Number', 'width' => .08, 'align' => 'center'),
                            array('text' => 'OR Date', 'width' => .08, 'align' => 'center'),
                            array('text' => 'Amount Paid', 'width' => .12, 'align' => 'center'),
                            array('text' => 'No. of Past Due', 'width' => .07, 'align' => 'center'),
                            array('text' => 'Net DM/CM', 'width' => .09, 'align' => 'center'),
                            array('text' => 'Balance', 'width' => .08, 'align' => 'center')
                        );
        
        $template = $engine->getTemplate();                         
        $template->setText('PHILIPPINE DAILY INQUIRER, INC.', 12);
        $template->setText('PLOUGH BACK (ADVERTISING) - '.$reportname, 10);
        ...
        $list = $this->mod_ploughback->getReport($datefrom, $dateto, $reporttype, $vdays, $agencyfrom, $c_clientfrom, $customerdata['id'] , $ac_client);   
        
        $winpaid = 0; $overpaid = 0; $unbalance = 0; $percentwin = 0; $percentover = 0; $percentunbalance = 0;
        //print_r2($list); exit;
        $gtotalgrossamt = 0; $gtotalagycommamt = 0;  $gtotalnetamt = 0; $gtotalamtpaid = 0; $gtotalnetdmcmpaid = 0; $gbalance = 0;     
        foreach ($list as $payee => $datarow) {
            
            $result[] = array(array('text' => $payee, 'align' => 'left'));
            $stotalgrossamt = 0; $stotalagycommamt = 0;  $stotalnetamt = 0; $stotalamtpaid = 0; $stotalnetdmcmpaid = 0; $sbalance = 0;     
    ...
    public function export() {
        ...
        $data['list'] = $this->mod_ploughback->getReport($datefrom, $dateto, $reporttype, $vdays, $agencyfrom, $c_clientfrom, $customerdata['id'] , $ac_client);
        $reportname = "";
        ...
        $html = $this->load->view('ploughback/export', $data, true); 
        $filename ="Plough_back".$reportname.".xls";
