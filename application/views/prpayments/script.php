...
<div id="lookup_view" title="PR Payment Lookup"></div>
<div id="singleinvoice_view" title="Single Invoice"></div>   
<div id="viewbooking_view" title="View All Booking Transaction for [AR and PTF]"></div>      
<script>

$("#prdelete").click(function() {
    var con = confirm("Are you sure you want to delete this provisional reciept?");
    
    if (con) {
        $.ajax({
            url: "<?php echo site_url('prpayment/prdelete'); ?>",
            type: "post",
            ...
            success: function(response) {
                ...
                window.location.href = "<?php echo base_url().'prpayment/load_prpayment/'.$this->uri->segment(3); ?>";              
                if ($response['stat'] == 'T') {
                    window.location.href = "<?php echo site_url('prpayment') ?>";      
                }
            }    
        });   
    }
...
$("#prcancelled").click(function() {
    var con = confirm("Are you sure you want to cancel this provisional reciept?");
    
    if (con) {
        $.ajax({
            url: "<?php echo site_url('prpayment/prcancelled'); ?>",
            type: "post",
            data: {ornum : '<?php echo $this->uri->segment(3); ?>'},
            success: function(response) {
                $response = $.parseJSON(response);
...
$("#orviewbooking").click(function() {
    $.ajax({
        url: "<?php echo site_url('prpayment/viewbookingview') ?>",
        type: "post",
        data: {},
        success: function(response) {
            $response = $.parseJSON(response);            
...
$("#single_invoce").click(function() {
    ...
    $.ajax({
        url: "<?php echo site_url('prpayment/singleinvoice') ?>",
        type: "post",
...
$("#action_newprpayment").click(function() {
		var confirmnew = confirm("Are you sure you want new pr payment?.");

		if (confirmnew) {
			window.location.href = "<?php echo base_url()?>prpayment";
		}
	});		
$("#prlookup").click(function() {
	$.ajax({
		url: "<?php echo site_url('prpayment/lookup') ?>",
		type: "post",
		data: {},
		success: function(response) {
			$response = $.parseJSON(response);
...
$("#prtype").change(function(){
	...
	if ($prtype == 2) {		
		$.ajax({
			url: "<?php echo site_url('prpayment/revenueBooking') ?>",
			type: "post",
			data: {},
			success: function(response) {
				$response = $.parseJSON(response);
...
$("#prsave").click(function() {
	var countValidate = 0;  
	var validate_fields = ['#prno', '#payeecode','#payeename', '#address1', 
...
function validatePRNumber($prno) {

	var amoutpaid = $("#amountpaid").val();
	var assignamount = $("#assignedamount").val();
	var x = amoutpaid.replace(",","");
	var z = assignamount.replace(",",""); 
	if (parseFloat(z) > parseFloat(x)) {
		alert("Assign amount must not be greather than amount paid!");
		return false;
	}
	$.ajax({
		url: "<?php echo site_url('prpayment/validatePRnumber') ?>",
		type: "post",
		data: {prno: $prno},
		success: function(response) {
			if (response == true) {
				alert('PR Number already exist!');
				$('#prno').val('').focus();
			} else {$("#form_savePrPayment").submit();}  
		}
	});
}
$("#prno").mask('99999999');
...
function recompute()
{
	...
	$.ajax({
		url: "<?php echo site_url('prpayment/getRecomputeValue') ?>",
		type: "post",
...
$('#retrieve').click(function(){
   $.ajax({
      url: "<?php echo site_url('prpayment/retrieveApplied') ?>",
      type: 'post',
      data: {type: $('#applicationtype').val(), code: $('#payeecode').val(), choose:$('#prchoose:checked').val(), mykeyid: "<?php echo $hkey ?>"},
      success:function(response) {
...
$('#filterinvoice').click(function(){
   ...
   $.ajax({
      url: "<?php echo site_url('prpayment/retrieveAppliedFilter') ?>",
      type: 'post',
      data: {type: $('#applicationtype').val(), code: $('#payeecode').val(), choose:$('#prchoose:checked').val(), mykeyid: "<?php echo $hkey ?>", f_invno: $f_invno},
      success:function(response) {
          var $response = $.parseJSON(response);
          if ($response['empty'] == '0') {
              alert('No record found!');
...
$("#notarialfee").autoNumeric();
var errorcssobj = {'background': '#E1CECE','border' : '1px solid #FF8989'};
var errorcssobj2 = {'background': '#E0ECF8','border' : '1px solid #D7D7D7'}; 
$("#add_paymenttype").click(function(){
	$.ajax({
		url: "<?php echo site_url('prpayment/paymenttype_view') ?>",
		type: "post",
...
$('#paymentapplied_view, #updatepaymentapplied_view, #singleinvoice_view, #viewbooking_view').dialog({
	autoOpen: false, 
	closeOnEscape: false,
	draggable: true,
	width: 670,    
...
/*$("#bank").change(function(){
    $.ajax({
        url: "<?php #echo site_url('prpayment/ajxGetBranch') ?>",
        type: 'post',
...
$("#evatstatus").click(function() {
	...
	} else { $(".radio_evat").attr("readonly", "readonly").val('0.00').removeData('autoNumeric'); recomputePercent();	}
});
...
$("#payeecode").autocomplete({                        
    source: function( request, response ) {
        $.ajax({
            url: "<?php echo site_url('prpayment/autocustomer') ?>",
            type: 'post',
