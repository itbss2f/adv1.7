        ...
        <div class="span12">
            <div class="head">
                <div class="isw-grid"></div>
                    <h1>List of Classified TPA's</h1>
                    <ul class="buttons">
                        <li>
                            <a href="#" class="isw-settings"></a>
                            <ul class="dd-list">
                                <?php if ($canADD) : ?>
                                <li><a href="#" id="newdata"><span class="isw-plus"></span> New TPA</a></li>                                                          
                                <?php endif; ?>
                                <li><a href="#" id="searchdata"><span class="isw-zoom"></span> Search TPA</a></li>                                                          
                            </ul>
                        </li>    
                    </ul>                           
                <div class="clear"></div>
            ...
            <thead>
                <tr>
                    <th width="10%">ID</th>
                    <th width="15%">TPA Code</th>
                    <th width="25%">TPA Name</th>                                      
                    <th width="10%">Action</th>                                                                
                </tr>
            </thead>
            <tbody>        
                <?php  foreach ($tpa as $row) : ?>    
                <tr>
...
$(function() {
    ...
    $('.edit').click(function() {
        var $id = $(this).attr('id');
        $.ajax({
          url: "<?php echo site_url('tpa/editdata') ?>",
          type: "post",
          data: {id: $id},
    ...
    $('.remove').click(function() {
        var $id = $(this).attr('id');
        var ans = confirm("Are you sure you want to remove this TPA?");    

        if (ans) {
            window.location = "<?php echo site_url('tpa/removeData') ?>/"+$id;
        }
    });

    $('#newdata').click(function(){        
       $.ajax({
          url: "<?php echo site_url('tpa/newdata') ?>",
          type: "post",
          data: {},
          success:function(response) {
             $response = $.parseJSON(response);
    ...
    $('#searchdata').click(function(){        
       $.ajax({
          url: "<?php echo site_url('tpa/searchdata') ?>",
          type: "post",
          data: {},
          success:function(response) {
             $response = $.parseJSON(response);
