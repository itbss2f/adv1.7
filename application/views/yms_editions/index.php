		...
		<div class="span12">
			<div class="head">
				<div class="isw-grid"></div>
					<h1>Yield Management System - Edition</h1>
					<ul class="buttons">
						<li>
							<a href="#" class="isw-settings"></a>
							<ul class="dd-list">
                                <?php if ($canADD) : ?>
								<li><a href="#" id="newdata"><span class="isw-plus"></span> New YMS - Edition</a></li>                    					                  
                                <?php endif; ?>
							</ul>
						</li>	
					</ul>           			    
...
$(function() {
	...
	$('.edit').click(function() {
		var $id = $(this).attr('id');
		$.ajax({
		  url: "<?php echo site_url('yms_edition/editdata') ?>",
		  type: "post",
		  data: {id: $id},
	...
	$('.remove').click(function() {
		var $id = $(this).attr('id');
		var ans = confirm("Are you sure you want to remove this YMS edition?");	

		if (ans) {
			window.location = "<?php echo site_url('yms_edition/removeData') ?>/"+$id;
		}
	});

	$('#newdata').click(function(){        
	   $.ajax({
		  url: "<?php echo site_url('yms_edition/newdata') ?>",
		  type: "post",
		  data: {},
		  success:function(response) {
			 $response = $.parseJSON(response);
