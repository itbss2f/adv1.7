...
<tr class='assign tbody' id='<?php echo $row['id'] ?>' datadid='<?php if(empty($row['did'])) { echo "x"; } else { echo @$row['did'];} ?>' databal='<?php echo $row['bal'] ?>' datatype='<?php echo $row['doctype'] ?>' datavat='<?php echo $row['vat_rate'] ?>'>   
...
$('.ass_amt').keyup(function(){
    ...
    if (parseFloat(amt) > parseFloat(balance)) {
        alert('Cannot applied!. Amount is greater than your balance!.');
