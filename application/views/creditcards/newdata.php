<div class="block-fluid">      
    <form action="<?php echo site_url('creditcard/save') ?>" method="post" name="formsave" id="formsave"> 
...
$("#save").click(function() {
    ...
    if (countValidate == 0) {
        $.ajax({
            url: "<?php echo site_url('creditcard/validateCode') ?>",
            type: "post",
            data: {code : $("#creditcard_code").val()},
            success: function(response) {
                if (response == "true") {                    
                       alert("Credit Card Code must be unique!.");
                       $('#creditcard_code').val('');
                   } else {
                       $('#formsave').submit();
                   }
