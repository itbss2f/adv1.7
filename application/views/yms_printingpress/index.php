	...
	<div class="row-fluid">
		...
		<div class="span12">
			<div class="head">
				...
				<h1>Yield Management System - Printing Press</h1>
				<ul class="buttons">
				<li>
				    <a href="#" class="isw-settings"></a>
				    <ul class="dd-list">
                        <?php if ($canADD) : ?>
					   <li><a href="#" id="newdata"><span class="isw-plus"></span> New Printing Press</a></li>                    					                  
                       <?php endif; ?>
				    </ul>
				</li>
				</ul>                        
...
$(function() {
	...
	$('.edit').click(function() {
		var $id = $(this).attr('id');
		$.ajax({
		  url: "<?php echo site_url('yms_printingpress/editdata') ?>",
		  type: "post",
		  data: {id: $id},
	...
	$('.remove').click(function() {
		var $id = $(this).attr('id');
		var ans = confirm("Are you sure you want to remove this printing press?");	

		if (ans) {
			window.location = "<?php echo site_url('yms_printingpress/removeData') ?>/"+$id;
		}
	});

	$('#newdata').click(function(){        
	   $.ajax({
		  url: "<?php echo site_url('yms_printingpress/newdata') ?>",
		  type: "post",
		  data: {},
		  success:function(response) {
			 $response = $.parseJSON(response);
