<div class="block-fluid">      
    <form action="<?php echo site_url('vat/save') ?>" method="post" name="formsave" id="formsave"> 
...
$("#save").click(function() {
    ...
    if (countValidate == 0) {
        $.ajax({
            url: "<?php echo site_url('vat/validateCode') ?>",
            type: "post",
            data: {code : $("#vat_code").val()},
            success: function(response) {
                if (response == "true") {                    
                       alert("Vat Code must be unique!.");
                       $('#vat_code').val('');
                   } else {
                       $('#formsave').submit();
                   }
