<div class="block-fluid">      
    <form action="<?php echo site_url('adsize/save') ?>" method="post" name="formsave" id="formsave"> 
...
$("#save").click(function() {
    ...
    if (countValidate == 0) {
        $.ajax({
            url: "<?php echo site_url('adsize/validateCode') ?>",
            type: "post",            
            data: {code : $("#adsize_code").val()},
            success: function(response) {
                if (response == "true") {                    
                       alert("Ad Size Code must be unique!.");
                       $('#adsize_code').val('');
                   } else {
                       $('#formsave').submit();
                   }
