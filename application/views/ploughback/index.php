...
$("#generatereport").click(function(response) {
    ...
    //window.location
    $("#source").attr('src', "<?php echo site_url('ploughback/generatereport') ?>/"+dateasfrom+"/"+dateasof+"/"+reporttype+"/"+vdays+"/"+agencyfrom+"/"+c_clientfrom+"/"+ac_agency+"/"+ac_client);     
...
$("#export").die().live ("click",function() {
    ...
    { 
    window.open("<?php echo site_url('ploughback/export/') ?>?dateasfrom="+dateasfrom+"&dateasof="+dateasof+"&reporttype="+reporttype+"&vdays="+vdays+"&agencyfrom="+agencyfrom+"&agencyto="+agencyto+"&c_clientfrom="+c_clientfrom+"&c_clientto="+c_clientto+"&ac_agency="+ac_agency+"&ac_client="+ac_client, '_blank');
        window.focus();
    }
