...
<div id="payment_view" title="OR Number">
    <div align="center">
     <button class="btn btn-large" type="button" style="width: 100px;" id="b_manual">Manual</button>
     <button class="btn btn-large btn-warning" type="button" style="width: 100px;" id="b_auto">Auto</button>
    </div>
</div>

<script> 
$('#b_manual').click(function() {
    $('#orno').removeAttr('readonly');    
    $('#orno').mask('99999999');
    $('#ortranstype').val('M');     
    $('#payment_view').dialog('close');  
});

$('#b_auto').click(function() {
    $('#orno').val('00000001');
    //$('#orno').unbind( "click", openPayment );
    $('#ortranstype').val('A');     
    $('#payment_view').dialog('close');       
});
...
$('#orno').one( "click", function() {
     $('#payment_view').dialog('open');  
});
...
$('#retrieve').click(function(){
   $.ajax({
      ...
      type: 'post',
      data: {artype: $('#ortype').val(),   type: $('#applicationtype').val(), code: $('#payeecode').val(), choose:$('#prchoose:checked').val(), mykeyid: "<?php echo $hkey ?>"},
      success:function(response) {
...
$('#filterinvoice').click(function(){
   ...
   $.ajax({
      ...
      type: 'post',
      data:{artype: $('#ortype').val(), type: $('#applicationtype').val(), code: $('#payeecode').val(), choose:$('#prchoose:checked').val(), mykeyid: "<?php echo $hkey ?>", f_invno: $f_invno},
      success:function(response) {
          var $response = $.parseJSON(response);
          if ($response['empty'] == '0') {
              alert('No record found!');
