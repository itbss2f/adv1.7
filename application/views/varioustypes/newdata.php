<div class="block-fluid">      
    <form action="<?php echo site_url('varioustype/save') ?>" method="post" name="formsave" id="formsave"> 
...
$("#save").click(function() {
    ...
    if (countValidate == 0) {
        $.ajax({
            url: "<?php echo site_url('varioustype/validateCode') ?>",
            type: "post",
            data: {code : $("#aovartype_code").val()},
            success: function(response) {
                if (response == "true") {                    
                       alert("Various Type Code must be unique!.");
                       $('#aovartype_code').val('');
                   } else {
                       $('#formsave').submit();
                   }
