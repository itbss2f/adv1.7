...
<div class="workplace">
    <div class="row-fluid">
        <div class="span12">
            <div class="head">
                <div class="isw-grid"></div>
                    <h1>Aging of Accounts Receivable (AE / AEAC All Rank/ AEAC Selected Rank/ AE Rank)</h1>
                    <ul class="buttons">
        ...
        <div class="block-fluid">                        
            <div class="row-form" style="padding: 2px 2px 2px 10px;">
                ...
                <div class="span2" style="margin-top:12px">
                    <select name="reporttype" id="reporttype">                    
                        ...
                        <option value="2">AEAC All Rank</option>                        
                        <option value="3">AEAC Selected Rank</option>                                                
                        <option value="4">AE Rank</option>                                                
                    </select>
                </div>
            ...
            <div class="row-form" id="ae" style="padding: 2px 2px 2px 10px;">  
                <div class="span2" style="width:80px;margin-top:12px">AE From</div>  
                <div class="span2" style="margin-top:12px">
                    <select name="ae1" id="ae1">
                    <?php foreach ($ae as $ae1) : ?>
                    <option value="<?php echo $ae1["empprofile_code"] ?>"><?php echo $ae1["empprofile_code"].' | '.$ae1["employee"] ?></option>
                    <?php endforeach; ?>
                    </select>
                </div>                                                        
                <div class="span2" style="width:80px;margin-top:12px">AE To</div>          
                <div class="span2" style="margin-top:12px">
                    <select name="ae2" id="ae2">
                    <?php foreach ($ae as $ae2) : ?>
                    <option value="<?php echo $ae2["empprofile_code"] ?>"><?php echo $ae2["empprofile_code"].' | '.$ae2["employee"] ?></option>
                    <?php endforeach; ?>
                    </select>
                </div>                                                        
                <div class="clear"></div>    
...
$("#generatereport").click(function(){
    ...
    if (countValidate == 0) {
        ...
        $.ajax({
            url: "<?php echo site_url('araging_report3/generatereport') ?>",
            type: "post",
            //data: {datefrom: datefrom, dateto: dateto, reporttype: reporttype, edition: edition, dummy: dummy, pay: pay, exclude: exclude},
            data: {},
            success:function(response) {
                    $("#source").attr('src', "<?php echo site_url('araging_report3/generatereport') ?>/"+datefrom+"/"+reporttype+"/"+ae1+"/"+ae2);
                    $(".c_loader").hide();
            }
        });        
    } else {            
