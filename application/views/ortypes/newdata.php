<div class="block-fluid">      
    <form action="<?php echo site_url('ortype/save') ?>" method="post" name="formsave" id="formsave"> 
...
$("#save").click(function() {
    ...
    if (countValidate == 0) {
        $.ajax({
            url: "<?php echo site_url('ortype/validateCode') ?>",
            type: "post",
            data: {code : $("#torf_code").val()},
            success: function(response) {
                if (response == "true") {                    
                       alert("Ortype Code must be unique!.");
                       $('#torf_code').val('');
                   } else {
                       $('#formsave').submit();
                   }
